...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        GeographyCountry = orm['geography.Country']
        GeographyCountry.objects.bulk_create([GeographyCountry(old_coco_id=x.id, country_name=x.country_name, start_date=x.start_date) for x in orm['dashboard.Country'].objects.all()])
        
        GeographyRegion = orm['geography.Region']
        GeographyRegion.objects.bulk_create([GeographyRegion(old_coco_id=x.id, region_name=x.region_name, start_date=x.start_date) for x in orm['dashboard.Region'].objects.all()])
        
        GeographyState = orm['geography.State']
        GeographyState.objects.bulk_create([GeographyState(old_coco_id=x.id, state_name=x.state_name, region=GeographyRegion.objects.get(old_coco_id=x.region_id), country=GeographyCountry.objects.get(old_coco_id=x.country_id), start_date=x.start_date) for x in orm['dashboard.State'].objects.all()])
        
        mDistrict = orm['geography.District']
        mDistrict.objects.bulk_create([mDistrict(old_coco_id=x.id, district_name=x.district_name, start_date=x.start_date, latitude=x.latitude, longitude=x.longitude, state=orm['geography.State'].objects.get(old_coco_id=x.state_id), partner=orm['partner.Partner'].objects.get(old_coco_id=x.partner_id)) for x in orm['dashboard.District'].objects.all()])
        
        mBlock = orm['geography.Block']
        mBlock.objects.bulk_create([mBlock(old_coco_id=x.id, block_name=x.block_name, start_date=x.start_date, district=mDistrict.objects.get(old_coco_id=x.district_id)) for x in orm['dashboard.Block'].objects.all()])
        
        mVillage = orm['geography.Village']
        mVillage.objects.bulk_create([mVillage(old_coco_id=x.id, village_name=x.village_name, block=mBlock.objects.get(old_coco_id=x.block_id), no_of_households=x.no_of_households, population=x.population, road_connectivity=x.road_connectivity, control=x.control, start_date=x.start_date, latitude=x.latitude, longitude=x.longitude, grade=x.grade) for x in orm['dashboard.Village'].objects.all()])
    
    
    def backwards(self, orm):
        orm['geography.Village'].objects.all().delete()
        orm['geography.Block'].objects.all().delete()
        orm['geography.District'].objects.all().delete()
        orm['geography.State'].objects.all().delete()
        orm['geography.Region'].objects.all().delete()
        orm['geography.Country'].objects.all().delete()
    ...
    complete_apps = ['dashboard', 'partner', 'geography']
    symmetrical = True
