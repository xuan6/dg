...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        Pap = orm['adoptions.PersonAdoptPractice']
        chunk = 1000
        dashboard_pap = orm['dashboard.PersonAdoptPractice'].objects.all()
        for i in range(0, dashboard_pap.count()/chunk + 1):
            dashboard_pap_slice = dashboard_pap[i*chunk:(i+1)*chunk]
            print "Pap Added %d" % (i*chunk)
            Pap.objects.bulk_create([Pap(old_coco_id=x.id, person=orm['people.Person'].objects.get(old_coco_id=x.person_id),
                                         video=orm['video.Video'].objects.get(old_coco_id=x.video_id),
                                         prior_adoption_flag=x.prior_adoption_flag, date_of_adoption=x.date_of_adoption,
                                         quality=x.quality, quantity=x.quantity, quantity_unit=x.quantity_unit, time_updated=x.time_updated,
                                         partner=orm['partner.Partner'].objects.get(old_coco_id=x.partner_id))
                                     for x in dashboard_pap_slice])
    ...
    complete_apps = ['dashboard', 'adoptions']
    symmetrical = True
